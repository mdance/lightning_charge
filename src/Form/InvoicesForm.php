...
/**
 * Provides the InvoicesForm class.
 */
class InvoicesForm extends FormBase {
  ...
  public function getFormId() {
    return 'lightning_charge_invoices_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $request = $args['request'] ?? $this->request;
    $invoices = $args['invoices'] ?? NULL;
    $form_options = $args['options'] ?? [];
    ...
    $show_operations = $form_options['show_operations'];
    $show_limit = $form_options['show_limit'];
    $show_order = $form_options['show_order'];

    $form['#show_order'] = $show_order;
    ...
    if ($show_limit) {
      ...
      $limit = $this->service->getDefaultLimit();
      $limit = $request->get($key, $limit);
    ...
    try {
      ...
      $limit = $request->query->get('limit', LightningChargeConstants::DEFAULT_LIMIT);
      ...
      foreach ($items as $item) {
        ...
        $option['id'] = $link;
        $option['amount'] = $item->getFormattedAmount();
        ...
        $result = $item->getSatoshisReceived();

        if ($result == 0) {
          $result = '';
        }

        $option['satoshis_received'] = $result;
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $default = $this->service->getDefaultLimit();
    $current = $this->request->query->get($key, $default);
